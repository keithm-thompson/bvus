...
class Api::UrlsController < ApplicationController
  ...
  def create
    uri = Url.create_uri(params['long_url'])
  ...
  def short_to_long
    short_hash = Url.extract_short_hash(params['short_url'])
    url = Url.where(short_url: short_hash)[0]
    if url && Url.valid_short_url_host?(params['short_url'])
      render status: 200, json: url.to_json
    else
